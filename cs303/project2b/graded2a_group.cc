...
using namespace std;
  ...
  */  
  ...
  /*-----------------------------------------------------
--------------------------------------------------------
Memory leak at GroupOfNumbers::mergesort() (group.cc:336)
-1 point
...
void GroupOfNumbers::mergesort()
{
	if(_total>0)// compare this
	{
		try
		{
			long *temp_group = new long[_total]; //allocate _total size, not _LIMIT. Otherwsie wastage of memory.
			mergesort(temp_group, 0, total() - 1);
...
Correction:
  void GroupOfNumbers::quicksort(long left, long right){
  	if(left + CutOff <= right){
  		...
  		insertion_sort(left,right+1); //insertion_sort you implemented takes one greater than the last position 
