	...
	void GroupOfNumbers::quick_select(long *temp_group, long left, long right, long k) const{
		if(left + CutOff <= right){
  		...
  		if(k <= i){
  			...
  			quick_select(temp_group, i, right, k); //sort large elements
  		}
...
insertion_sort
---------------------
line 243: for(j = i; j > 0 && temp < temp_group[j-1]; j--){
use: for(j = i; j > left && temp < temp_group[j-1]; j--){

sort_select
------------------------
memory leak:
long* temp_group = new long[_total]; //should be freed. add line  "delete[] temp_group;" before the line "return true;"
point: -1
...
for(i = k; i < _total; i++){
			if(temp_group[i] < temp_group[k-1]){
				temp1 = temp_group[i];
				for(int j = 0; j < k+c; j++){ //In every checking you should check k or more items. Otherwise no improvment than sort_select.
					if(temp1 < temp_group[j]){
						temp2 = temp_group[j];
						temp_group[j] = temp1;
						temp1 = temp2;
...
for(i = k; i < _total; i++)
{
    if (temp_group[i] < temp_group[k-1])
    {
		...
		for(int j = k-1; j >0; j--)
		{
			...
			{
				break; // As first k items are sorted. Don't need to check further if (temp_group[j] < temp_group[j-1])==false.
			}
		}
    }
}
...
output error:
line 294: for(int i = 0; i < k; i++ ){// should be i<k-1 i.e. for(int i = 0; i < k-1; i++ )
line 301: use kth_smallest = temp_group[1]; No need to use extra variable temp.

Point: -2

line 285 to 298:
long* temp_group = new long[_total]; //percolate_down2 uses code weiss 6.12. weiss 6.12 starts array from position 1.
  	for(int i = 0; i < _total; i++){
  		temp_group[i] = _group[i];
  	}
		long currentSize = _total;
		for( int i = _total / 2; i >= 0; i-- ) // Should be for( int i = _total / 2; i > 0; i-- ). As weiss 6.12 count node from 1.
			percolate_down2(temp_group, i, _total );

		long tempMin;
		for(int i = 0; i < k; i++ ){
			tempMin= temp_group[1];
			temp_group[ 1 ] = temp_group[ currentSize-- ]; //currentSize=_total will create an exception.
			percolate_down2(temp_group, 1, currentSize);
		}
...
line 395 to 397: 
 ...
 Should be:
  	...
  	quick_select(temp_group, i+1, right, k);
 }
Point: -1
