  ...
  */  
  ...
  void GroupOfNumbers::insertion_sort(long left, long right){
  	...
  	for(int p = left; p < right; p++){
	  	long tmp = _group[p];
		//coments by grader(-1): make suren the index j won't be outside the range[left,right]
  ...
  void GroupOfNumbers::quick_choose_first(long left, long right){
  	if(left + CutOff <= right){
  		...
  		long i = left, j = right + 1;
  		for(;;){
			/*---------------comments by grader(-1.5)
			When i==right, the code will access _group[right+1],
			when j==left, the code will access _group[left-1].
			*/
  ...
  void GroupOfNumbers::quick_choose_middle(long left, long right){
   	if(left + CutOff <= right){
		...
		store pivot before partition, endpoints can be used:
		swap(_group[left],_group[(left+right)/2])
		or swap(_group[right,_group[(left+right)/2]) 
  		-------------------*/
		long i = left, j = right-1;
  		for(;;){
  			while(_group[++i] < pivot){}
