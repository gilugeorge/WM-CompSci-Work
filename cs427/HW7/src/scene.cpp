...
scene::~scene(void)
{
  ...
  for(unsigned int i=0; i < _bvh.size(); i++)         if(_bvh[i])         delete _bvh[i];
}
...
const sceneGraph_base& scene::addSceneGraphObject(const std::vector<triangle>& triangles, const material& mat, bool computeBVH)
{
  // create mesh
  _mesh.push_back(new triangleMesh(triangles, mat));

  // check if we need to compute a BVH?
  if(computeBVH)
    _bvh.push_back(new boundingVolumeHierarchy(*_mesh.back()));
  else 
    _bvh.push_back(NULL);

  // add to node
  _nodes.push_back(new sceneGraphObject(*(_mesh.back()), *(_bvh.back()) ));
  return *(_nodes.back());
}


const sceneGraph_base& scene::addSceneGraphObject(const std::string& filename, const material& mat, bool computeBVH)
{
  // create mesh
