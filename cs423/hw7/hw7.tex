 ...
 \item (20 points) Problem 2.30 (a) and (d) on page 131.
  \begin{description}
   \item a) Use the pumping lemma to show that the following language is not context free: \\
	...
	Pumping $s$, all $p$s must be equal or a $p`$ that is different from $p$ will result, rendering $s \notin A$. Therefore,
	...
	1. one $p$ and a maximum length of p. \\
	2. at most 2 $p$s no matter where $vxy$ spans $0^p1^p0^p1^p$. \\
   ...
   \item d) Use the pumping lemma to show that the following language is not context free: \\
	...
	\begin{itemize}
	 ...
	 \item[Case 2:] $vxy$ is contained in $b^p\#a^p$. With $s = uv^0xy^0z$, there are fewer $b$s on the left side and fewer $a$s on
	      the right side. Thus $s \notin B$.
