 ...
 \item (16 points) Problem 1.49 (a) and (b) on page 90.\\ a) Strings accepted in B are:
  ...
  b) Show the C is not regular: \\
    \begin{description}
     ...
     \item We choose $s = 1^p01^p$, $|s| = 2p + 1 \geq p$. Since $|ab| \leq p$, $|ab|$ will 
     represent $1^p$ and $c$ will represent $01^p$. When $i = 0$, $b$ is removed from $abc$.
     This resulting string $ac$ will have $k$ decremented by at least one since $|b| > 0$, but $y$
     will contain the same amount of $1s$ since $c$ is unchanged. THus $y$ will have more than $k$ $1s$
