 ...
 */
public abstract class MazeBuilder implements Runnable, Serializable {
	...
	/**This method is within MazeBuilderFalstad
	 * 
	 */
	...
	protected void setExitPosition(int remotex, int remotey) {
		...
		exitPosition[ 0 ] = remotex;
		exitPosition[ 1 ] = remotey;
		System.out.println("exit position set to zero: " + remotex + " "
				+ remotey + " " + bit + ":"
				+ cells.hasMaskedBitsFalse(remotex, remotey, bit));
	}
	...
	private Seg findPartitionCandidate(Vector<Seg> sl) {
		...
		for (int i = 0; i < sl.size(); i += skip) {
			...
			partiters++;
//			if ((partiters & 31) == 0) {
//				if (maze.increasePercentage(partiters*100/expected_partiters)) //TODO: possibly have to reimplement this later
//				{
...
//			}
	...
	public void build(Maze mz, int w, int h, int roomct, int pc) {
		random = new Random(); //TODO: set seed for maze
		width = w;
		height = h;
		maze = mz;
		rooms = roomct;
	...
	public void build(Maze mz, int w, int h, int roomct, int pc, int seed) {
		random = new Random(seed); //TODO: set seed for maze
		width = w;
		height = h;
		maze = mz;
		rooms = roomct;
