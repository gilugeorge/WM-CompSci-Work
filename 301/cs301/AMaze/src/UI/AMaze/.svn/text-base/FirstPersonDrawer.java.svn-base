...
public class FirstPersonDrawer implements Serializable{
	 ...
	 * @param rset
	 * @param ang
	 */                    // \/change from Graphics to GrahicsWrapper TODO
	public void redrawPlay(GraphicsWrapper gc, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		gWrap.setColor(0); //TODO
		gWrap.fillRect(0, 0, view_width, view_height/2);
		// draw dark gray background on upper half
		gWrap.setColor(1); //TODO
		gWrap.fillRect(0, view_height/2, view_width, view_height/2);
		// set color to white and draw what ever can be seen from the current position
		gWrap.setColor(2); //TODO
	...
	private void drawrect(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
		gWrap.paint.setColor(Color.GREEN);	
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			Path p = new Path(); 		//TODO: new path code to take over for fillPolygon()
			p.moveTo(xps[0], yps[0]);
			for(int i = 0; i < 4; i++){
				p.lineTo(xps[i], yps[i]);
			}
			p.close();
			gWrap.gc.drawPath(p, gWrap.paint);
